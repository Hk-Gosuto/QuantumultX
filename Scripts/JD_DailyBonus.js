...
async function all(cookie, jrBody) {
  ...
  switch (stop) {
    ...
    //京东金融 京豆双签
      ...
      await JingDongTurn(Wait(stop));
      await JDFlashSale(Wait(stop));
      await JingDongCash(Wait(stop));
      await JDMagicCube(Wait(stop), 2);
      await JingDongGetCash(Wait(stop));
      await JingDongSubsidy(Wait(stop));
      await JingDongShake(Wait(stop));
      ...
      await JingRongDoll(Wait(stop), "JTDouble", "京东金贴-双签", "1DF13833F7");
      await JDUserSignPre(
        Wait(stop),
        "JDStory",
        "京东失眠-补贴",
